...
describe('whatsappTemplate', () => {
  ...
  describe('gerarMensagemWhatsApp', () => {
    it('deve gerar mensagem com template padrão', () => {
      ...
      expect(mensagem).toContain('adega');
      expect(mensagem).toContain('osbrothersadega@gmail.com');
    });
    ...
    it('deve incluir PIX no template padrão', () => {
      const mensagem = gerarMensagemWhatsApp(TEMPLATE_PADRAO, clienteTeste, lancamentoTeste);
  ...
  describe('validarTemplate', () => {
    it('deve validar template correto', () => {
      const resultado = validarTemplate('Olá {cliente}, seu débito é {valor} com pix {pix}');
    ...
    it('deve aceitar template com todas as variáveis válidas', () => {
      const resultado = validarTemplate('{cliente} {valor} {data} {descricao} {pix}');
