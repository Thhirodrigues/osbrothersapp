...
export async function notificarNovoLancamento(
  ...
  return enviarEmail({
    para: 'osbrothersadega@gmail.com',
    assunto: `${tipoLabel} registrado - ${clienteNome}`,
    html,
    bcc: 'victorhgs26@gmail.com',
  });
}
 ...
 */
export async function notificarPagamentoCliente(
  emailCliente: string,
  clienteNome: string,
  valor: number
): Promise<boolean> {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #388e3c;">Pagamento Confirmado</h2>
      
      <div style="background-color: #e8f5e9; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #388e3c;">
        <p>Olá, <strong>${clienteNome}</strong>!</p>
        <p>Seu pagamento foi registrado com sucesso.</p>
        <p><strong>Valor:</strong> <span style="color: #388e3c; font-size: 18px; font-weight: bold;">${valorFormatado}</span></p>
        <p><strong>Data/Hora:</strong> ${new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</p>
      </div>

      <p style="color: #666; font-size: 12px;">
        Esta é uma confirmação automática do Os Brothers Adega.
      </p>
    </div>
  `;
  ...
  return enviarEmail({
    para: [emailCliente],
    assunto: `Pagamento confirmado - ${valorFormatado}`,
    html,
    bcc: 'victorhgs26@gmail.com',
  });
}
